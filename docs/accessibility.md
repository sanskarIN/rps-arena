...
- Statistics trend controls/labels;
- About links.
...
Achievement lock/trophy symbols are accompanied by achievement title and description text.
...
Use Material theme color roles rather than hard-coded text/status colors. Winners, errors, timers, locks, warnings, profile selection, and trend results must include text or symbols so color is never the only signal.
...
- [ ] Verify W/L/D trend items announce Win/Loss/Draw in order.
